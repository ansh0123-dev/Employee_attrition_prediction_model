...
def _handle(resp: requests.Response) -> dict:
    try:
        ...
        body = {"success": False, "message": f"Unexpected server response ({resp.status_code})"}
...
def register(name: str, email: str, password: str) -> dict:
    try:
        ...
        return {"success": False, "message": f"Could not reach backend: {e}", "_status": 0}
...
def logout(token: str) -> dict:
    try:
        r = requests.post(
            f"{BACKEND_URL}/api/auth/logout", headers=_auth_headers(token), timeout=TIMEOUT
        )
        return _handle(r)
    except requests.exceptions.RequestException as e:
        return {"success": False, "message": f"Could not reach backend: {e}", "_status": 0}
...
def get_me(token: str) -> dict:
    try:
        r = requests.get(
            f"{BACKEND_URL}/api/auth/me", headers=_auth_headers(token), timeout=TIMEOUT
        )
        return _handle(r)
    except requests.exceptions.RequestException as e:
        return {"success": False, "message": f"Could not reach backend: {e}", "_status": 0}
